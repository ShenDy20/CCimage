...
with open('./zz2d/train2d/train.txt', 'r') as f:
    train = f.readlines()
with open('./zz2d/test2d/test.txt', 'r') as g:
    test = g.readlines()
...
for i in range(len(train)):
    ...
    img_path='./zz2d/train2d/'+str(i)+'.txt'
...
for i in range(len(test)):
    ...
    label=test[i].split()
    label=list(map(float,label))
    img_path='./zz2d/test2d/'+str(i)+'.txt'
    img = np.loadtxt(img_path,delimiter=' ')
    img=torch.tensor(img)
    #print(img.shape)
    img=img.reshape(1,32,32)
...
def train_and_valid(model, loss_function, optimizer, epochs=30):
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            temp=0
            point_acc=0
            pred_label_locate = torch.argsort(outputs, descending=True)[:, 0:20].cuda()
