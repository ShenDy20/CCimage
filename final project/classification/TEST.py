...
new_m = torch.load('vit4.pt')
img_path='./zzmore/train/151.txt'
img = np.loadtxt(img_path,delimiter=' ')
img=torch.tensor(img)

img=img.reshape(1,64,32)
img=transform(img)
img=img.reshape(1,1,64,32).cuda()
img=img.to(torch.float32)
predict = new_m(img)
pred_label_locate = torch.argsort(predict, descending=True)[:, 0:25]
for i in range(predict.shape[0]):
    temp_label = torch.zeros(1, predict.shape[1]).cuda()
    temp_label[0,pred_label_locate[i]] = 1

predict=temp_label.cpu()
result1 = predict.detach().numpy()
