...
with open('./2d/train/train.txt', 'r') as f:
    train = f.readlines()
with open('./2d/test/test.txt', 'r') as g:
    test = g.readlines()
...
def train_and_valid(model, loss_function, optimizer, epochs=30):
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            for m in range(sh[0]):
                for k in range(sh[1]):
                    if labels.data[m][k]==1:
                        total+=1
                    if labels.data[m][k]==1 and torch.sigmoid(outputs.data[m][k])>0.1:
                        temp+=1
                        point_acc+=1
                    if labels.data[m][k]==0 and torch.sigmoid(outputs.data[m][k])<0.1:
                        temp+=1
            acc=temp/(outputs.data.size(1))
            p_acc=point_acc/total
            train_p_acc+=p_acc
            train_acc += acc
