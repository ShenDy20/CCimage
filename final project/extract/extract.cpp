...
int main()
{
	...
	std::shuffle(shuffle1.begin(), shuffle1.end(), std::default_random_engine(seed));
	std::shuffle(shuffle2.begin(), shuffle2.end(), std::default_random_engine(seed));




    for (int HH=-5;HH<6;HH++)
        for (int j = -3; j < 1; j++)
        {
            string file1 = "./C"+to_string(HH)+to_string(j)+ "0.txt";
            ifstream in(file1);
            string line;
			...
			for (int I = 0; I < 50; I++)
			{
				...
				{
					temp.push_back(testrdm[tt] + 3000);
				}
				double DD[32][32];
				for (int J = 0; J < 32; J++)
				{
				...
				//testlabels[cnt2]=label;
				testlabels[shuffle2[cnt2]] = label;
				cnt2++;
				fclose(fp2);
			}
			...
			for (int I = 0; I < 500; I++)
			{
				...
				//trainlabels[cnt1] = label;
				trainlabels[shuffle1[cnt1]] = label;
				cnt1++;
				fclose(fp1);
			}
